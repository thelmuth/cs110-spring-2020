...
class Player:
    ...
    def __init__(self, name, deck):
        """Creates a new player. Note that a Deck object must be passed to
        the player, so that they can draw from it."""
        self._name = name
        self._deck = deck
        self._hand = Hand()
    ...
    def draw_card(self):
        """Draws a card out of deck into hand."""
        card = self._deck.draw_one_card()
        self._hand.take_into_hand(card)

    def hit_or_stay(self):
        """Asks the user whether to hit or stay, given their current value."""
...
class Dealer(Player):
    """A player class for a dealer, who has rules defining when to hit or stay.
    Only redefines the hit_or_stay method of the Player class."""

    def hit_or_stay(self):
        """Uses dealer rules to hit or stay.
        Hits on 16 or less, stays on 17 or more."""
...
def main():
    ...
    players = [veronica, arnold, penelope, dealer]
    ...
    for player in players:
        ...
        if value > 21:
            ...
            print("{} had a hand of {} with value {}.".format(player.get_name(),
                                                              ...
                                                              value))
            if value > best_value:
                best_hand = player.get_name()
                best_value = value

    print("The winner is {}!".format(best_hand))
